...
exports.FloatingActionButton = React.forwardRef(function (props, ref) {
    ...
    var buttonId = React.useMemo(function () { return kendo_react_common_1.guid(); }, []);
    var dir = kendo_react_common_1.useDir(elementRef, props.dir);
    var isRtl = dir === 'rtl';
    var listId = React.useMemo(function () { return kendo_react_common_1.guid(); }, []);
    ...
    var dispatchPopupEvent = React.useCallback(function (dispatchedEvent, isOpen) {
        ...
        kendo_react_common_1.dispatchEvent(isOpen ? onOpen : onClose, dispatchedEvent, getImperativeHandle(), undefined);
    ...
    var handleClick = React.useCallback(function (event) {
        ...
        if (!items && onClick) {
            kendo_react_common_1.dispatchEvent(onClick, event, getImperativeHandle(), undefined);
        }
        else {
            setOpen(!open);
            setFocused(true);
    ...
    var dispatchItemClickEvent = React.useCallback(function (dispatchedEvent, index) {
        ...
        if (!items[index].disabled) {
            kendo_react_common_1.dispatchEvent(onItemClick, dispatchedEvent, getImperativeHandle(), {
    ...
    var handleKeyDown = React.useCallback(function (event) {
        ...
        kendo_react_common_1.dispatchEvent(onKeyDown, event, getImperativeHandle(), undefined);
    }, [onKeyDown, focusedIndex, setFocusedIndex, isRtl, setOpen]);
    var buttonClassNames = React.useMemo(function () { return kendo_react_common_1.classNames('k-fab', "k-fab-" + themeColor, "k-fab-" + shape, {
        'k-fab-sm': size === 'small',
    ...
    return (React.createElement(React.Fragment, null,
        React.createElement("button", __assign({ ref: elementRef, id: id || buttonId, role: items ? 'menubutton' : 'button', type: 'button', "aria-disabled": disabled, "aria-expanded": items ? open : undefined, "aria-haspopup": items ? true : false, "aria-label": (text || '') + " floatingactionbutton", "aria-owns": items ? listId : undefined, "aria-activedescendant": focusedIndex >= 0 && items ? listId + "-" + focusedIndex : undefined, tabIndex: kendo_react_common_1.getTabIndex(tabIndex, disabled), accessKey: accessKey, dir: dir, disabled: disabled, className: buttonClassNames, style: style, onClick: handleClick, onMouseDown: handleMouseDown, onFocus: handleFocus, onBlur: handleBlur, onKeyDown: handleKeyDown }, others),
            icon ?
                React.createElement("span", { role: "presentation", className: kendo_react_common_1.classNames("k-fab-icon k-icon k-i-" + icon) }) :
                iconClass ?
                    React.createElement("span", { role: "presentation", className: iconClass }) :
                    null,
            text && React.createElement("span", { className: "k-fab-text" }, text)),
        React.createElement(kendo_react_popup_1.Popup, { ref: popupRef, anchor: elementRef.current, show: open, animate: popupSettings.animate, popupClass: kendo_react_common_1.classNames('k-popup-transparent k-fab-popup', popupSettings.popupClass), anchorAlign: popupSettings.anchorAlign || utils_1.getAnchorAlign(align, isRtl), popupAlign: popupSettings.popupAlign || utils_1.getPopupAlign(align, isRtl), style: { boxShadow: 'none' } },
            React.createElement("ul", { ref: listRef, role: 'menu', "aria-labelledby": id, id: listId, className: kendo_react_common_1.classNames('k-fab-items', {
